...
const items = [
  { icon: DollarSign, stat: '2%',   title: 'Platform Fee',  sub: 'or minimum 10,000 UZS per transaction',  note: 'Simple, transparent pricing with no hidden costs' },
  { icon: RefreshCw,  stat: '3',    title: 'Max Revisions', sub: 'Fixed brief prevents endless changes',    note: 'Clear expectations protect both sides' },
  { icon: ShieldCheck,stat: '100%', title: 'Audit Trail',   sub: 'Every action logged and timestamped',     note: 'Complete transparency for all parties' },
]
          ...
          {items.map((item) => {
            ...
            return (
              <div key={item.title} className="bg-white border border-gray-100 rounded-2xl p-6 lg:p-8 flex flex-col items-center text-center gap-3 lg:gap-4">
                <div className="w-14 h-14 lg:w-16 lg:h-16 rounded-2xl bg-[#009cde]/10 flex items-center justify-center">
                  <Icon size={24} className="text-[#009cde] lg:hidden" />
                  <Icon size={28} className="text-[#009cde] hidden lg:block" />
                </div>
